    ...
    stages {
        ...
        stage('Container') {
            steps {
                ...
                dir('contenedor') {
                    withCredentials([usernamePassword(credentialsId: 'fran-jimenez', passwordVariable: 'password', usernameVariable: 'usuario')]) {
                        git 'https://github.com/fran-trassierra/app.git'
                        sh('''
                            cp ../codigo/webapp/target/webapp.war .
                            git add .
                            git config --global user.email 'fjjimenez@iestrassierra.com'
                            git config --global user.name  'Fran Jiménez'
